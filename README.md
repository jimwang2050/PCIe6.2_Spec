...
| **返回目录** | 每节末尾 `[⬆️ 返回目录](#sec-N-0)` 跳转 |
| **中英对照表格** | `<table>` 双列布局：EN (白底) \| ZH (灰底) |
| **PAGE_BREAK 标记** | 保留 `<!-- 📄 Page XXX -->` 用于页码定位 |

### 排版规范 (v2.1 — 2026-07-01)

| 规范 | 说明 |
|------|------|
| **中文逗号** | 中文语境统一使用全角 `，` (已全库替换 11,822 处) |
| **中英混排空格** | 中文与英文/数字间保留半角空格 (如 `TLP 序列号`) |
| **交叉引用** | 统一使用 `§ Section X.Y` 格式（中文技术文档惯例），保留原 spec 交叉引用 |
| **图注格式** | EN: `> **Figure N-XX.** Title` / ZH: `> **图 N-XX.** 中文标题`，使用 blockquote `>` 前缀 |
| **IMP NOTE 格式** | 中英对照：EN `> **IMPLEMENTATION NOTE:**` / ZH `> **实现说明 (IMPLEMENTATION NOTE):**` |
| **表格回退** | Markdown 表格优先；含 `\|` 复杂单元格改用 HTML `<table>` 或注记说明 |
| **术语黑名单** | `有序集合→有序集` `热插入→热插拔` `流量控制→流控` `SR-ROV→SR-IOV` |
