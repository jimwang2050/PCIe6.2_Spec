...
|:------:|:------|:-------|:----:|
| 1 | PCI Express Link |  | — |
| 2 | Example PCI Express Topology |  | — |
| 3 | Logical Block Diagram of a Swi… |  | — |
| 4 | High-Level Layering Diagram |  | — |
...
> <img src="figures/chapter_01/fig_0128_1_tight.png" width="700">
> <img src="figures/chapter_01/fig_0127_1.png" width="700">

> **图 1-1.** PCI Express 链路
> <img src="figures/chapter_01/fig_0128_1_tight.png" width="700">
> <img src="figures/chapter_01/fig_0127_1.png" width="700">
...
<!-- 📍 Figure 1-5 原图未单独抽取，相关内容见 Figure 1-4 分层图 -->
...
<!-- 📍 图 1-5 原图未单独抽取，相关内容见 Figure 1-4 分层图 -->
...
Each of these layers is divided into two sections: one that processes outbound (to be transmitted) information and one that processes inbound (received) information. The three layers are:

- **Transaction Layer**
- **Data Link Layer**
- **Physical Layer**

The following sub-sections describe the functions and services supported by each of these layers.

</td>
<td style="background-color:#e8e8e8">

这三层又各自划分为两个部分：一部分处理出向 (outbound，将要发送) 信息，另一部分处理入向 (inbound，已接收) 信息。三个逻辑层为：
...
**符号 (Symbol) 与特殊有序集 (Ordered Set) 生成：**
...
Inter-Layer interfaces are the boundaries between adjacent layers where services are requested and provided. The interfaces between layers are well-defined and standardized, allowing implementations to optimize the design of each layer independently. PCI Express defines two inter-layer interfaces:

- **Transaction/Data Link Interface:** between the Transaction Layer and the Data Link Layer
- **Data Link/Physical Interface:** between the Data Link Layer and the Physical Layer

</td>
<td style="background-color:#e8e8e8">

层间接口是相邻两层之间请求与提供服务的边界。层之间的接口定义清晰且标准化，允许实现独立优化每一层的设计。PCI Express 定义了两个层间接口：

- **事务层/数据链路层接口 (Transaction/Data Link Interface)**：位于事务层与数据链路层之间
- **数据链路层/物理层接口 (Data Link/Physical Interface)**：位于数据链路层与物理层之间
