...
|:------:|:------|:-------|:----:|
| 1 | Generic Platform Configuration |  | — |
| 2 | Generic Platform Configuration… |  | — |
| 3 | Generic Platform Configuration… |  | — |
| 4 | Example Multi-Function Device |  | p.1528 |
| 5 | Example SR-IOV Single PF Capab… |  | p.1529 |
| 6 | Example SR-IOV Multi-PF Capabl… |  | p.1531 |
| 7 | Example SR-IOV Device with Mul… |  | p.1533 |
| 8 | Example SR-IOV Device with a M… |  | p.1534 |
| 9 | BAR Space Example for Single B… |  | p.1536 |
| 10 | SR-IOV Extended Capability |  | p.1541 |
| 12 | SR-IOV Capabilities Register |  | p.1542 |
| 13 | SR-IOV Control Register |  | p.1545 |
| 14 | SR-IOV Status |  | p.1549 |

## 📊 本章表格 (Tables)

| Table | Title | 表标题 | Page |
|:-----:|:------|:-------|:----:|
| 1 | VF Routing ID Algorithm | VF 路由 ID 算法 | p.1537 |
| 1 | VF 路由 ID 算法 (VF Routing ID Alg… |  | p.1537 |
| 2 | SR-IOV Extended Capability Hea… | SR-IOV 扩展能力结构头部 | p.1542 |
| 2 | SR-IOV 扩展能力结构头部 (SR-IOV Extend… |  | p.1542 |
| 3 | SR-IOV Capabilities Register | SR-IOV 能力寄存器 | p.1542 |
| 3 | SR-IOV 能力寄存器 (SR-IOV Capabilit… |  | p.1542 |
| 4 | SR-IOV Control Register | SR-IOV 控制寄存器 | p.1545 |
| 4 | SR-IOV 控制寄存器 (SR-IOV Control R… |  | p.1545 |
| 5 | SR-IOV Status | SR-IOV 状态 | p.1549 |
| 5 | SR-IOV 状态 (SR-IOV Status) |  | p.1549 |
| 8 | BAR Offsets | BAR 偏移 | p.1554 |
| 8 | BAR 偏移 (BAR Offsets) |  | p.1554 |
| 9 | SR-IOV Usage of PCI Standard C… | SR-IOV 对 PCI 标准能力结构的使用 | p.1555 |
| 9 | SR-IOV 对 PCI 标准能力结构的使用 (SR-IOV… |  | p.1555 |
| 10 | SR-IOV Usage of PCI Express Ex… | SR-IOV 对 PCI Express 扩展能力结构的使用 | p.1556 |
| 10 | SR-IOV 对 PCI Express 扩展能力结构的使用… |  | p.1556 |
...
> **图 9-1.** Generic Platform Configuration
...
> **图 9-2.** Generic Platform Configuration 含 VI 与 Multiple SI
...
> **图 9-3.** Generic Platform Configuration 含 SR-IOV 与 IOV Enablers
...
> **图 9-4.** Example Multi-Function Device
...
> **图 9-5.** Example SR-IOV Single PF Capable Device
...
> **图 9-6.** Example SR-IOV Multi-PF Capable Device
...
> **图 9-7.** Example SR-IOV Device 含 Multiple Bus Numbers
...
> **图 9-8.** Example SR-IOV Device 含 Mixture Function Types
...
> **图 9-9.** BAR Space Example Single BAR Device
...
> **图 9-10.** SR-IOV Extended Capability
...
| 位位置 | 寄存器描述 | 属性 |
|--------------|----------------------|------------|
| 15:0 | PCI Express 扩展能力结构 ID (PCI Express Extended Capability ID)——此字段是 PCI-SIG 定义的 ID 号，用于指示扩展能力结构的性质和格式。SR-IOV 扩展能力结构的扩展能力结构 ID 为 0010h。 | RO |
...
> **图 9-12.** SR-IOV 能力寄存器 (Capabilities Register)
...
> **不支持更大的 VF Tag Completer 位 (NO VF LARGER-TAG COMPLETER SUPPORTED BITS)**
> 不存在 VF 14 位或 VF 10 位 Tag Completer 支持位。如果某个 PF 支持一个或两个大标签 Completer 能力，则其所有关联 VF 必须支持关联的 Tag Completer 能力(如 14 位 Tag Completer 支持和 10 位 Tag Completer 支持中所述)。这有助于避免 PCIe 层级中的复杂性——某些 Completer 支持大标签能力而某些不支持。
...
> **图 9-13.** SR-IOV Control Register
...
> **实现说明 (IMPLEMENTATION NOTE):**
> **VF MSE 与 VF Enable (VF MSE AND VF ENABLE)**
> 当 VF Enable 清零时,VF 内存空间将响应"不支持的请求" (Unsupported Request)。因此，当 VF Enable 清零时,VF MSE 为"无关"状态;但是，软件可以在设置 VF Enable 之前，在对 VF BARn 寄存器进行编程之后选择设置 VF MSE。
